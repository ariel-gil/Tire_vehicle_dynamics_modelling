...
if kia==0  %the interpolation is not super accurate, since it takes 2 end cases, not all 4 (min min, min max, max min, max max). for that, the ifs are added. but needs to be fixed. 
nfyx=(kfz)*(nfymax-nfymin)+nfymin;
elseif kfz==0
    nfyx=(kia)*(nfymax-nfymin)+nfymin;
else
nfyx=(kfz)*(kia)*(nfymax-nfymin)+nfymin
end 
